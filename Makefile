...
LDFLAGS=-ldflags "-X $(INTERNAL).gitCommit=$(GIT_COMMIT) -X $(INTERNAL).buildTime=$(BUILD_TIME) -X $(INTERNAL).goVersion=$(GO_VERSION) -s -w"
...
SOFIA_HOME?=$(HOME)/.sofia
WORKSPACE_DIR?=$(SOFIA_HOME)/workspace
...
## build-all: Build sofia for all platforms
build-all: generate
...
## install: Install sofia to system and copy builtin skills
install: build
...
uninstall-all:
	...
	@rm -rf $(SOFIA_HOME)
	@echo "Removed workspace: $(SOFIA_HOME)"
...
help:
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.sofia/workspace)"
