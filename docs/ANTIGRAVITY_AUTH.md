...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in Sofia.
...
10. [Creating a New Provider in Sofia](#creating-a-new-provider-in-sofia)
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<SofiaConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- Sofia codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.sofia/auth.json`:
...
## Creating a New Provider in Sofia

Sofia providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/sofia/cmd_auth.go`:
...
sofia auth login --provider your-provider
...
sofia agent -m "Hello" --model your-model
```
...
export SOFIA_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export SOFIA_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `sofia auth login --provider antigravity`
...
- Check auth profile storage: `~/.sofia/auth.json`
- Re-run `sofia auth login --provider antigravity`
